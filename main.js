// js/main.js - VERSIÓN FINAL CON TODOS LOS JUEGOS
...
const ANIMALS = [
    { id: 'perro', es: 'Perro', en: 'Dog', emoji: '🐶', bg: '#E67E22', sound: 'Guau guau' },
    { id: 'gato', es: 'Gato', en: 'Cat', emoji: '🐱', bg: '#E74C3C', sound: 'Miau' },
    { id: 'vaca', es: 'Vaca', en: 'Cow', emoji: '🐮', bg: '#27AE60', sound: 'Muuu' },
    { id: 'pato', es: 'Pato', en: 'Duck', emoji: '🦆', bg: '#3498DB', sound: 'Cuac cuac' },
    { id: 'leon', es: 'León', en: 'Lion', emoji: '🦁', bg: '#F1C40F', sound: 'Roaar' },
    { id: 'elefante', es: 'Elefante', en: 'Elephant', emoji: '🐘', bg: '#9B59B6', sound: 'Barritar' },
    { id: 'mono', es: 'Mono', en: 'Monkey', emoji: '🐒', bg: '#16A085', sound: 'Uh uh ah' },
    { id: 'conejo', es: 'Conejo', en: 'Rabbit', emoji: '🐰', bg: '#E91E8C', sound: 'Silencio' }
];
...
// ============================================
// RENDER: COLORES, VOCALES, ALFABETO, ETC.
// ============================================
...
function renderStoryScene() {
    ...
    viewer.innerHTML = `
        <div style="background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);border-radius:24px;padding:24px;color:#fff;min-height:200px;">
            <div style="font-size:48px;text-align:center;">${scene.match(/^.{1,2}/)?.[0] || '📖'}</div>
            <div style="font-size:20px;font-weight:900;text-align:center;margin:16px 0;">${scene}</div>
            <div style="display:flex;justify-content:space-between;margin-top:16px;">
                <span>${currentStoryIndex + 1} / ${currentStory.escenas.length}</span>
                <div>
                    ${currentStoryIndex > 0 ? `<button onclick="window.storyPrev()" style="padding:8px 20px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">⬅️</button>` : ''}
                    ${currentStoryIndex < currentStory.escenas.length - 1 ? `<button onclick="window.storyNext()" style="padding:8px 20px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">➡️</button>` : `<button onclick="window.closeStory()" style="padding:8px 20px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">✅ Leído</button>`}
                </div>
            </div>
        </div>
    `;
...
export function startMatchGame() {
    ...
    area.innerHTML = `
        <div style="background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);border-radius:24px;padding:24px;color:#fff;text-align:center;">
            ...
            <button onclick="window.startMatchGame()" style="margin-top:12px;padding:8px 24px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">🔄 Reiniciar</button>
            <button onclick="window.closeGame()" style="margin-top:12px;margin-left:8px;padding:8px 24px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">✕ Cerrar</button>
        </div>
    `;
}
...
function playColorRound() {
    ...
    area.innerHTML = `
        <div style="background:linear-gradient(135deg,#f093fb 0%,#f5576c 100%);border-radius:24px;padding:24px;color:#fff;text-align:center;">
            ...
            <button onclick="window.startColorGame()" style="margin-top:12px;padding:8px 24px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">🔄 Reiniciar</button>
            <button onclick="window.closeGame()" style="margin-top:12px;margin-left:8px;padding:8px 24px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">✕ Cerrar</button>
        </div>
    `;
}
...
export function startNumberGame() {
    ...
    area.innerHTML = `
        <div style="background:linear-gradient(135deg,#2C3E50 0%,#3498DB 100%);border-radius:24px;padding:24px;color:#fff;text-align:center;">
            ...
            <p style="font-size:14px;opacity:0.8;">Toca los números en orden del 1 al 10</p>
            ...
            <button onclick="window.startNumberGame()" style="margin-top:12px;padding:8px 24px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">🔄 Reiniciar</button>
            <button onclick="window.closeGame()" style="margin-top:12px;margin-left:8px;padding:8px 24px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">✕ Cerrar</button>
        </div>
    `;
}
...
export function startWheelGame() {
    ...
    area.innerHTML = `
        <div style="background:linear-gradient(135deg,#FF6B6B 0%,#FFE66D 100%);border-radius:24px;padding:24px;text-align:center;color:#2d2d2d;">
            ...
            <div id="wheel-result" style="font-size:20px;font-weight:900;margin-top:16px;min-height:40px;">Toca la ruleta para girar</div>
            <button onclick="window.closeGame()" style="margin-top:12px;padding:8px 24px;border-radius:50px;border:none;background:rgba(0,0,0,0.1);color:#2d2d2d;font-weight:900;cursor:pointer;">✕ Cerrar</button>
        </div>
    `;
}
...
function renderHangman() {
    ...
    area.innerHTML = `
        <div style="background:linear-gradient(135deg,#2C3E50 0%,#3498DB 100%);border-radius:24px;padding:24px;color:#fff;text-align:center;">
            ...
            <button onclick="window.startHangmanGame()" style="margin-top:12px;padding:8px 24px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">🔄 Nueva palabra</button>
            <button onclick="window.closeGame()" style="margin-top:12px;margin-left:8px;padding:8px 24px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">✕ Cerrar</button>
        </div>
    `;
}
...
export function startTriviaGame() {
    ...
    triviaQuestions = [
        { pregunta: '¿Qué color es el cielo?', opciones: ['Rojo', 'Azul', 'Verde'], correcta: 1 },
        { pregunta: '¿Cuántas patas tiene un perro?', opciones: ['2', '3', '4'], correcta: 2 },
        { pregunta: '¿Qué animal dice "Miau"?', opciones: ['Perro', 'Gato', 'Vaca'], correcta: 1 },
        { pregunta: '¿Cuánto es 2 + 3?', opciones: ['3', '4', '5'], correcta: 2 },
        { pregunta: '¿Qué forma tiene una pelota?', opciones: ['Cuadrado', 'Círculo', 'Triángulo'], correcta: 1 }
    ];
...
function showTriviaQuestion() {
    ...
    if (triviaIndex >= triviaQuestions.length) {
        area.innerHTML = `
            <div style="background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);border-radius:24px;padding:24px;color:#fff;text-align:center;">
                ...
                <button onclick="window.startTriviaGame()" style="margin-top:12px;padding:8px 24px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">🔄 Jugar de nuevo</button>
                <button onclick="window.closeGame()" style="margin-top:12px;margin-left:8px;padding:8px 24px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">✕ Cerrar</button>
            </div>
        `;
        return;
    ...
    area.innerHTML = `
        <div style="background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);border-radius:24px;padding:24px;color:#fff;text-align:center;">
            ...
            <div style="font-size:20px;font-weight:900;margin:16px 0;">${q.pregunta}</div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;max-width:280px;margin:0 auto;">
            ...
            <button onclick="window.closeGame()" style="margin-top:12px;padding:8px 24px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">✕ Cerrar</button>
        </div>
    `;
}
...
window.checkTriviaAnswer = function(selected, correct) {
    ...
    if (selected === correct) {
        ...
        message.textContent = `❌ Era: ${triviaQuestions[triviaIndex].opciones[correct]}`;
...
export function startMath(type) {
    ...
    area.innerHTML = `
        <div style="background:linear-gradient(135deg,#f093fb 0%,#f5576c 100%);border-radius:24px;padding:24px;color:#fff;text-align:center;">
            <h3>🧮 ${type === 'suma' ? 'Suma' : type === 'resta' ? 'Resta' : 'Mixto'}</h3>
            ...
            <button onclick="window.startMath('${type}')" style="margin-top:12px;padding:8px 24px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">🔄 Nueva</button>
            <button onclick="window.closeGame()" style="margin-top:12px;margin-left:8px;padding:8px 24px;border-radius:50px;border:none;background:rgba(255,255,255,0.2);color:#fff;font-weight:900;cursor:pointer;">✕ Cerrar</button>
        </div>
    `;
}
...
window.openVideo = function(videoId, titulo) {
    ...
    area.innerHTML += `
        <div style="margin-bottom:16px;">
            ...
            <button onclick="window.closeVideo()" style="margin-top:4px;padding:4px 12px;border-radius:20px;border:none;background:#EB5757;color:#fff;font-weight:900;cursor:pointer;">✕ Cerrar</button>
        </div>
    `;
    addStars(2);
};
